...
const WATER_EFFECT_FALLBACK_REFRESH_RATE = 120;
const waterEffectTime = signal(0);
let waterEffectPoll: PollHandle | null = null;
let waterEffectStartedAtMs = Date.now();

const WATER_SURFACE_UNIFORMS = {
  ...
  reflection_gain: 700.0,
  reflection_cutoff: 0.012,
  reflection_intensity: 200000.0,
  water_tint: 0.92,
};
...
function maxOutputRefreshRate(): number {
  const refreshRates = COMPOSITOR.output.outputs
    ...
    .filter(
      (refreshRate): refreshRate is number =>
        typeof refreshRate === "number" && refreshRate > 0,
    );

  if (refreshRates.length === 0) {
    return WATER_EFFECT_FALLBACK_REFRESH_RATE;
  }

  return Math.max(...refreshRates);
}

function restartWaterEffectClock(): void {
  const elapsedMs = waterEffectTime.peek() * 1000;
  waterEffectStartedAtMs = Date.now() - elapsedMs;

  waterEffectPoll?.cancel();
  waterEffectPoll = createPoll(1000 / maxOutputRefreshRate(), () => {
    waterEffectTime.value = (Date.now() - waterEffectStartedAtMs) / 1000;
  });
}

restartWaterEffectClock();

COMPOSITOR.onDisable((event) => {
  waterEffectPoll?.cancel();
  waterEffectPoll = null;
...
COMPOSITOR.window.composition = (window: WaylandWindow) => {
  ...
  const backgroundShader = compileEffect({
    ...
    pipeline: [
      dualKawaseBlur({ radius: 4, passes: 2 }),
      shaderStage(loadShader("./src/water-terminal.frag"), {
        uniforms: WATER_SURFACE_UNIFORMS,
      }),
    ],
  });
